...
Rambit's function is to save consecutive areas of memory or single or compound files produced by the Assembler/Editor cartridge at the 3300-3600 baud rate mentioned. Single stage load tapes follow Rambit's conventions already, so converting these is a matter of using the utility's 'L' command to load the original, and the 'S' command to save the converted program to a blank tape. A verification facility allows the checking of the new tape's loading ability. A variety of other commands, many of which are similar to those in the Assembler/Editor cartridge, allow one to display and alter memory.
