...
; Enhanced BASIC to assemble under Tass, $ver 1.05
...
Usrjmp		= $DE		; USR function JMP address
...
TWidth		= $E1		; BASIC terminal width byte
Iclim		= $E2		; input column limit
Itempl		= $E3		; temporary integer low byte (for GOTO etc)
Itemph		= Itempl+1	; temporary integer high byte (for GOTO etc)
...
Ibuffs		= $0401	; start of input buffer
...
Imode			= $62		; input mode flag, $00=INPUT, $98=READ
...
TK_PRINT		= $9F		; PRINT token

TK_CLEAR		= $A2		; CLEAR token
...
TK_NOT		= $B0		; NOT token
TK_STEP		= $B1		; STEP token
TK_UNTIL		= $B2		; UNTIL token
...
TK_CHRS		= $D9		; CHR$ token

TK_BINS		= $DB		; BIN$ token

TK_VPTR		= $E1		; VARPTR token
...
LAB_2D99
	...
	ASL	A			; shift test byte left (now $AA)
	STA	(Itempl),Y		; save via temporary integer
	CMP	(Itempl),Y		; compare via temporary integer
...
LAB_11D6
	...
	STA	Temp_2		; save MOD(block length/$100) byte
	...
	LDA	Obendl		; get block end low byte
	SEC				; set carry for subtract
	SBC	Temp_2		; subtract MOD(block length/$100) byte
	...
	SEC				; set carry for subtract
LAB_11F3
	LDA	Nbendl		; get destination end low byte
	SBC	Temp_2		; subtract MOD(block length/$100) byte
...
LAB_1280
	STX	Bpntrl		; set BASIC execute pointer low byte
	STY	Bpntrh		; set BASIC execute pointer high byte
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory
...
LAB_1319
	...
	STA	ut1_pl		; set line start pointer low byte
	STY	ut1_ph		; set line start pointer high byte
	CLC				; clear carry for possible later add
LAB_1325
	LDY	#$01			; index to high byte of next line pointer
	LDA	(ut1_pl),Y		; get it
	BNE	LAB_132E		; there is a program (or not reached end yet) so we must
					; go and fix the pointers

					; else we just
	JMP	LAB_127D		; wait for Basic command (no "Ready")
...
LAB_1330
	...
	LDA	ut1_ph		; get line start pointer high byte
	...
	BCC	LAB_1325		; go do next line (carry always clear)
...
; returns Bpntrl pointing to buffer-1
...
LAB_13AC
	...
	BEQ	LAB_13EC		; if null save byte then exit (saves >4000 cycles/line)

	CMP	$20			; is it a space ?
	BEQ	LAB_13EC		; if so save byte then continue crunching
	...
	CMP	#'?			; compare with "?" character (PRINT)
	BNE	LAB_13C4		; branch if not "?"

	LDA	#TK_PRINT		; else keyword token is TK_PRINT
	BNE	LAB_13EC		; always branch, save byte then continue crunching

					; wasn't [SPACE], " or ?
LAB_13C4
	CMP	#$30			; compare with "0"
	BCC	LAB_13CC		; branch if < (continue crunching)

	CMP	#$3C			; compare with "<"
	BCC	LAB_13EC		; branch if < (was 0123456789:;)
					; go save byte then continue crunching

					; gets here with next character not numeric or ; or :
LAB_13CC
	STY	csidx			; copy buffer save index
	LDY	#<LAB_KEYT		; get keyword table low address
	STY	ut2_pl		; save pointer low byte
	LDY	#>LAB_KEYT		; get keyword table high address
	STY	ut2_ph		; save pointer high byte
	LDY	#$00			; clear table pointer
	STY	Tindx			; clear word index
	STX	TempB			; save buffer read index
	BEQ	LAB_13D8		; jump into loop (branch always)

LAB_13D6
	INY				; next table byte
	BNE	LAB_13D7		; skip high byte INC if no rollover

	INC	ut2_ph		; increment table pointer high byte
LAB_13D7
	INX				; next buffer byte
LAB_13D8
	LDA	Ibuffs,X		; get byte from input buffer
	EOR	(ut2_pl),Y		; Exclusive OR table byte (was SEC, SBC)
	BEQ	LAB_13D6		; go compare next if match

	CMP	#$80			; was it end marker match ?
	BNE	LAB_1417		; branch if not (not found keyword)

					; else found keyword
	ORA	Tindx			; OR with word index (+$80 in A makes token)
LAB_13EA
...
LAB_13EC
	...
	BEQ	LAB_142A		; branch if was null [EOL]
...
LAB_1401
	SEC				; set carry for subtract
	SBC	#(TK_REM-$3A)	; subtract REM token offset
...
LAB_1417
	LDX	TempB			; compare has failed, restore buffer index
	INC	Tindx			; increment keyword index (next keyword)
...
LAB_141B
	...
	BNE	LAB_141E		; skip inc if no overflow
...
LAB_SHLN
	...
	BEQ	LAB_144D		; if = go check low byte

	BCS	LAB_145F		; else if temp < this line, exit (passed line#)
...
LAB_14BD
	...
	.BYTE $20, LAB_GBYT, $00 		;	JSR	LAB_GBYT		; scan memory
...
; to make LIST in line executable we need to remove the next two lines ####
...
; also, to make it look prettier, we need this moved before the LDA (Baslnl),Y ####
...
; to finish off making LIST in line executable we need to change the exit to .. ####

	RTS
;	JMP	LAB_GBYT		; get BASIC byte and return
...
LAB_152E
	...
	LDY	#$00			; clear index byte
LAB_1540
	DEX				; decrement token #
	BEQ	LAB_154B		; if now found go do printing
...
LAB_1543
	...
	INY				; increment table index
	BNE	LAB_1544		; skip INC if no overflow

	INC	ut2_ph		; increment table pointer high byte
LAB_1544
	PLP				; restore status
	BPL	LAB_1543		; loop until keyword end marker

	BMI	LAB_1540		; go test if this is required keyword

					; found keyword (it's the next one)
LAB_154B
	LDA	(ut2_pl),Y		; get byte from table
	BMI	LAB_1508		; go restore index, mask byte & print if
					; byte was end marker
	JSR	LAB_PRNA		; else go print the character
	INY				; increment table index
	BNE	LAB_154B		; do next if no overflow

	INC	ut2_ph		; increment table pointer high byte
	BNE	LAB_154B		; go get next character (branch always)
...
LAB_FOR
	...
	JSR	LAB_11A1		; search the stack for FOR or GOSUB activity
	BNE	LAB_1567		;.branch if FOR (this var) not found

					;.FOR (this var) was found so first we dump the old one
	TXA				; copy index
	ADC	#CR_CHAR			; add FOR structure size-2
	TAX				; add to index
...
LAB_1602
	EOR	#$80			; normalise token (was SEC, SBC)
	BPL	LAB_1609		; branch if token

	JMP	LAB_LET		; else go do implied LET

LAB_1609
	CMP	#(TK_TAB-$80)	; compare normalised token with TAB
	BCS	LAB_15D9		; branch if A>=TAB (do syntax error, then warm start)
					; only tokens before TAB can start a line
	ASL	A			; *2 (2 bytes per vector)
	TAY				; copy to index
...
LAB_LOOP
	STA	Frnxth		; save following token
...
LoopAlways
	...
	STA	Bpntrh		; save BASIC execute pointer high byte
	JMP	DoAgain		; go do DO again

					; clear stack & back to interpreter loop
LoopDone
	PLA				; dump current line low byte
	PLA				; dump current line high byte
...
LAB_SNBL
	...
	STX	Srchc			; store alt search character
	LDY	#$00			; set search character = [EOL]
	STY	Asrch			; store search character
LAB_1725
	LDA	Asrch			; get search character
	LDX	Srchc			; get alt search character
	STA	Srchc			; make search character = alt search character
	STX	Asrch			; make alt search character = search character
LAB_172D
	LDA	(Bpntrl),Y		; get next byte
	...
	BNE	LAB_172D		; go get next character
...
LAB_GFPN
	...
	STX	Itemph		; clear temporary integer high byte
LAB_1785
	BCS	LAB_177E		; return if carry set (end of scan, character was not 0-9)

	SBC	#$2F			; subtract $30 ($2F+carry) from byte
	STA	Temp3			; store #
	LDA	Itemph		; get temporary integer high byte
	STA	ut1_pl		; save it for now
	...
	LDA	Itempl		; get temporary integer low byte
	ASL	A			; *2 low byte
	ROL	ut1_pl		; *2 high byte
	ASL	A			; *2 low byte
	ROL	ut1_pl		; *2 high byte (*4)
	ADC	Itempl		; + low byte (*5)
	STA	Itempl		; save it
	LDA	ut1_pl		; get high byte temp
	ADC	Itemph		; + high byte (*5)
	STA	Itemph		; save it
	ASL	Itempl		; *2 low byte (*10d)
	ROL	Itemph		; *2 high byte (*10d)
	LDA	Itempl		; get low byte
	ADC	Temp3			; add #
	STA	Itempl		; save low byte
	BCC	LAB_17B3		; branch if no overflow to high byte

	INC	Itemph		; else increment high byte
LAB_17B3
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory
...
LAB_17B5
	STA	TempB			; save +/-1 pointer low byte
LAB_17B7
	...
	JSR	LAB_UFAC		; unpack memory (AY) into FAC1
	LDA	TempB			; get +/-1 pointer low byte
	...
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment and scan memory
...
LAB_17FB
	...
	LDA	#$AC			; set descriptor pointer low byte
...
LAB_NoSt
	JSR	LAB_RTST		; check for space on descriptor stack then put string address
...
LAB_1866
	...
	LDX	#<(Ibuffs-1)	; set X to buffer start-1 low byte
	LDY	#>(Ibuffs-1)	; set Y to buffer start-1 high byte
...
LAB_18A2
	...
	BNE	LAB_18B7		; branch if not (was SPC)
...
LAB_18B7
	JSR	LAB_18E0		; print " "
	DEX				; decrement count
	BNE	LAB_18B7		; loop if not all done
...
LAB_1880
	...
	LDA	#CR_CHAR		; restore the character (and set the flags)
LAB_1886
...
LAB_1904
	LDA	Imode			; get input mode flag, $00=INPUT, $98=READ
...
LAB_1934
	...
	JSR	LAB_INLN		; print "? " and get BASIC input
	LDA	Ibuffs		; get first byte from buffer
	BNE	LAB_1953+1	; branch if not null input (this is a strange branch, it
...
LAB_1953
	...
	STA	Imode			; set input mode flag, <=$7F=INPUT, $98=READ
...
LAB_195B
	...
	STY	Bpntrh		; set BASIC execute pointer high byte
	.BYTE $20, LAB_GBYT, $00 ; JSR	LAB_GBYT		; scan memory
	BNE	LAB_1985		; branch if not null

					; pointer was to null entry
	BIT	Imode			; test input mode flag, $00=INPUT, $98=READ
...
LAB_1985
	...
	LDA	#$44			; other search character is ","
	CLC				; clear carry for add
LAB_1999
...
LAB_19B6
	.BYTE $20, LAB_GBYT, $00 ; JSR	LAB_GBYT		; scan memory
	BEQ	LAB_19C2		; branch if null (last entry)

	CMP	#$44			; else compare with ","
...
LAB_1A03
	...
	LDX	Imode			; get input mode flag, $00=INPUT, $98=READ
...
LAB_1A56
	...
	INX				; +2
	INX				; +3
	INX				; +4 (point to STEP var)
	TXA				; copy to A
	INX				; +1
	INX				; +2
	INX				; +3
	INX				; +4
	INX				; +5 (point to TO var)
	STX	ut2_pl		; save stack pointer to TO var for compare
	...
	TSX				; get stack pointer back
	SEC				; set carry for subtract
	SBC	LAB_STAK+8,X	; subtract step sign
...
LAB_1A9B
	...
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; else increment & scan memory
...
LAB_1AFA
	...
	ADC	Dtypef		; add data type flag, $FF=string, $00=numeric
	BNE	LAB_1B0B		; branch if not string or not + token
...
LAB_1B34
	DEC	Bpntrl		; decrement BASIC execute pointer low byte
	LDY	#(TK_LT-TK_PLUS)*3; set offset to last operator entry
	STA	comp_f		; save new compare function flag
...
LAB_1B43
	...
	PHA				; onto stack
	JSR	LAB_1B56		; function will return here, then the next RTS will call
					; the function
...
LAB_1B56
	LDA	FAC1_s		; get FAC1 sign (b7)
	LDX	LAB_OPPT,Y		; get precedence byte
...
LAB_GVAL
	LDA	#$00			; clear byte
	STA	Dtypef		; clear data type flag, $FF=string, $00=numeric
LAB_1BA4
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory
...
; this is the hook for get value from line, it catches the number check to include
; numbers starting with "$" and "%"
	...
	BCS	LAB_1C18		; get (var), return value in FAC1 & $ flag
	...
	BEQ	LAB_1BA4		; branch if + token (+1 = 1 so ignore leading +)

					; it wasn't any sort of number so ...
	CMP	#$22			; compare with "
	BNE	LAB_1BD0		; branch if not open quote
...
LAB_1BEE
	...
	BCC	LAB_1BF5		; branch if less than SGN token

					; else was a function token
	BCS	LAB_1C27		; go set up function references (branch always)
...
LAB_1C11
	LDY	#(TK_GT-TK_PLUS)*3; set offset from base to > operator
LAB_1C13
	PLA				;.
	PLA				;.
	JMP	LAB_1B1D		;.
...
LAB_1C18
	...
	STY	FAC1_3		; save address high5^ byte in FAC1 mantissa3
	LDX	Dtypef		; get data type flag, $FF=string, $00=numeric
	BNE	LAB_1C25		; if string then return (does RTS)
...
LAB_1C27
	...
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory
	CPX	#(TK_CHRS-$80)*2+1; compare function offset to CHR$ token offset+1
	BCC	LAB_1C51		; branch if <HEX$ (can not be =)

; get value from line .. continued
; was HEX$, BIN$, VARPTR, LEFT$, RIGHT$ or MID$ so..

	CPX	#(TK_BINS-$80)*2+1; compare function offset to BIN$ token offset+1
	BCC	LAB_BHSS		; branch if <VARPTR (can not be =)

	CPX	#(TK_VPTR-$80)*2+1; compare function offset VARPTR token offset+1
	BCC	LAB_1C54		; branch if <LEFT$ (can not be =)

; get value from line .. continued
; was LEFT$, RIGHT$ or MID$ so..

	JSR	LAB_1BFE		; scan for "(" , else do syntax error, then warm start
	JSR	LAB_EVEX		; evaluate (should be string) expression
...
LAB_CFAC
	...
	.BYTE $20, LAB_GBYT, $00 ;JSR	LAB_GBYT		; get next BASIC byte
...
LAB_1C51
	JSR	LAB_1BF5		; evaluate expression within parentheses
...
LAB_1C56
	...
	.BYTE $20, Fnxjmp, $00  ;JSR	Fnxjmp		; do function call
	JMP	LAB_CTNM		; check if source is numeric & RTS, else do type mismatch
...
LAB_1CAE
	...
	BEQ	LAB_1CD6		; branch if strings = length
...
LAB_GVAR
	...
	.BYTE $20, LAB_GBYT, $00 ; JSR	LAB_GBYT		; scan memory (1st character)
LAB_1D10
...
LAB_1D1F
	...
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory (2nd character)
...
LAB_1D2E
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory (3rd character)
...
LAB_1D83
	SBC	#$7B			; subtract "z"+1
	SEC				; set carry
	SBC	#$85			; subtract $A5 (restore byte)
...
LAB_1D8B
	...
	CMP	#<LAB_1C18+2		; compare with expected calling routine return low byte
...
; found the array and we're not dimensioning it so we must find an element in it
	...
	BNE	LAB_1E85		; if wrong do array bounds error (could do "Wrong dimensions"
					; error here .. if we want another, different, error message)

	JMP	LAB_1F28		; found array so go get element
					; (could jump to LAB_1F28 as all LAB_1F24 does is take Dimcnt
					; and save it at (Astrtl),Y which is already the same or we
					; would have taken the BNE)
...
LAB_FRE
	LDA	Dtypef		; get data type flag, $FF=string, $00=numeric
	BEQ	LAB_1FB4		; branch if numeric
...
LAB_AYFC
	...
	JMP	LAB_27E3		; set exp=X, clearFAC1_3, normalise & RET
...
LAB_201E
	JSR	LAB_200B		; check FNx syntax
	LDA	func_h		; get function pointer high byte
	PHA				; push it
	LDA	func_l		; get function pointer low byte
	PHA				; push it
	JSR	LAB_1BF5		; evaluate expression within parentheses
	JSR	LAB_CTNM		; check if source is numeric, else do type mismatch
...
; was string array so ...
	...
	ADC	ut1_pl		; add array start low byte
	STA	ut1_pl		; save data start low byte
	BCC	LAB_21C2		; branch if no overflow

	INC	ut1_ph		; else increment data start high byte
LAB_21C2
	LDX	ut1_ph		; get element pointer high byte
LAB_21C4
...
LAB_21EC
	...
	BCC	LAB_2206		; if <, step pointer to next string & RET
...
LAB_22BA
	STA	ut1_pl		; save string pointer low byte
...
LAB_CHRS
	...
	PLA				; dump return address (return via get value from line)
	PLA				; dump return address (return via get value from line)
	JMP	LAB_RTST		; check for space on descriptor stack then put string address
					; and length on descriptor stack & update stack pointers
...
NoString
	PLA				; dump return address (return via get value from line call)
	PLA				; dump return address
	JMP	LAB_RTST		; check for space on descriptor stack then put string address
					; and length on descriptor stack & update stack pointers
...
LAB_SADD
	JSR	LAB_EVST		; evaluate string
	TYA				; string address high byte to A
	LDY	ut1_pl		; string address low byte to Y
...
LAB_23C5
	...
	LDX	ut1_ph		; get string pointer high byte
	STX	Bpntrh		; save as BASIC execute pointer high byte
	BCC	LAB_23DD		; branch if no high byte increment

	INX				; increment string end high byte
LAB_23DD
	STX	ut2_ph		; save string end high byte
	LDY	#$00			; set index to $00
	LDA	(ut2_pl),Y		; get string end byte
	PHA				; push it
	LDA	#$00			; clear A
...
LAB_SWAP
	...
	CMP	Dtypef		; compare with var2 data type
...
LAB_ADD
	...
	JMP	LAB_279B		; FAC1 was zero so copy FAC2 to FAC1 & return
...
LAB_24F8
	...
	STA	FAC1_1		; save FAC1 mantissa1
	JMP	LAB_2528		; test & normalise FAC1 for C=0/1
...
LAB_2680
	...
	JMP	LAB_24F5		; save FAC1 exponent & return
...
LAB_276B
	...
	.byte	$2C			; makes next line BIT LAB_14A2
...
LAB_289E
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory (ignore + or get next number)
LAB_28A1
	BCC	LAB_28FE		; branch if numeric character
...
LAB_2953
	...
	JSR	LAB_296B		; print null terminated string
...
LAB_2ABF
	...
	JSR	LAB_27F8		; compare FAC1 with (AY)
	BNE	LAB_2AD9		; branch if FAC1 <> (AY)
...
LAB_2B84
	...
	STY	Cptrh			; save count pointer high byte
LAB_2B88
	JSR	LAB_276B		; pack FAC1 into numexp
...
; RND(n), 31 bit version. make n=0 for next number in sequence or n<>0 to get next
; number in sequence after seed n
...
NextPRN
	...
	LDA	Rbyte4		; get PRNG extra byte
	ROR	A			; bit 32 -> carry
	ROR	A			; bit 31 -> carry
	BCC	Ninc1			; skip increment if =0

	INX				; else increment bit count
Ninc1
	AND	#$08			; mask bit 30 (bit 28 before shifts)
	BEQ	Ninc2			; skip increment if =0

	INX				; else increment bit count
Ninc2
	TXA				; copy bit count to A
	LSR	A			; shift b0 to carry
	ROR	Rbyte1		; shift PRNG most significant byte
	ROR	Rbyte3		; shift PRNG middle byte
	ROR	Rbyte2		; shift PRNG least significant byte
	ROR	Rbyte4		; shift PRNG extra byte
...
TST_OK
	.BYTE $20, LAB_IGBY, $00 ;JSR	LAB_IGBY		; update BASIC execute pointer (to character past ")")
	LDA	#$00			; clear A
	SEC				; set the carry
T_Bits
	ROL	A			; shift bit
...
LAB_CHEX
	...
	SEC				; set carry for subtract
	SBC	#'a			; subtract "a"
	BCC	LAB_EXCH		; exit if <"a"

	CMP	#$06			; compare normalised with $06 (max+1)
	BCS	LAB_EXCH		; exit if >"f"

	ADC	#$3A			; convert to nibble+"0"
LAB_ISHN
	SBC	#$2F			; convert from ASCII (carry is clear)
	TAX				; save nibble
	LDA	FAC1_e		; get FAC1 exponent
	BEQ	LAB_MLTE		; skip multiply if zero

	CLC				; clear carry for add
	ADC	#$04			; add four to exponent (*16)
	BCS	LAB_MLTO		; do overflow error if > $FF
...
LAB_MLTE
	...
	JSR	LAB_2912		; evaluate new ASCII digit
	JMP	LAB_CHEX		; go do next character
...
LAB_CBIN
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory
	BCS	LAB_EXCH		; branch if not numeric character

	CMP	#'2			; compare with "2" (max+1)
	BCS	LAB_EXCH		; exit if >="2"
...
LAB_MLBT
	...
	TXA				; restore bit
	JSR	LAB_2912		; evaluate new ASCII digit
	JMP	LAB_CBIN		; go do next character
...
LAB_NMI
	...
	SEC				; set carry for subtract
	SBC	#TK_CLEAR		; compare with token for CLEAR (A = $00 if = TK_CLEAR)
...
LAB_SNMI
	...
	.BYTE $20, LAB_IGBY, $00 ; JSR	LAB_IGBY		; increment & scan memory (past token)
	JSR	LAB_GFPN		; get fixed-point number into temp integer
	LDA	Smeml			; get start of mem low byte
	LDX	Smemh			; get start of mem high byte
	JSR	LAB_SHLN		; search Basic for temp integer line number from AX
...
LAB_MAXN
	...
	BPL	LAB_MAXN		; branch if no swap to do

	LDA	FAC2_1		; get FAC2 mantissa1
	ORA	#$80			; set top bit (clear sign from compare)
	STA	FAC2_1		; save FAC2 mantissa1
	JSR	LAB_279B		; copy FAC2 to FAC1
	BEQ	LAB_MAXN		; go do next (branch always)
...
LAB_MINN
	JSR	LAB_PHFA		; push FAC1, evaluate expression,
					; pull FAC2 & compare with FAC1
	BMI	LAB_MINN		; branch if no swap to do

	BEQ	LAB_MINN		; branch if no swap to do

	LDA	FAC2_1		; get FAC2 mantissa1
	ORA	#$80			; set top bit (clear sign from compare)
	STA	FAC2_1		; save FAC2 mantissa1
	JSR	LAB_279B		; copy FAC2 to FAC1
	BEQ	LAB_MINN		; go do next (branch always)
...
LAB_MMEC
	...
	PLA				; dump return address high byte
	.BYTE $20, LAB_IGBY, $00 ;JSR	LAB_IGBY		; update BASIC execute pointer (to chr past ")")
	RTS				;
...
LAB_PHFA
	...
	.BYTE $20, LAB_IGBY, $00 ;JSR	LAB_IGBY		; scan & get next BASIC byte (after ",")
	JSR	LAB_EVNM		; evaluate expression & check is numeric,
					; else do type mismatch
...
LAB_NSTT
	...
	.BYTE $20, LAB_GBYT, $00 ;JSR	LAB_GBYT		; get BASIC byte back
...
LAB_NOFC
	...
	BNE	LAB_SQE2		; if odd only 1 shift first time
...
LAB_FTBL = *-(TK_SGN-$80)*2	; offset for table start
LAB_FTBM = LAB_FTBL+$01
	...
	.word	LAB_POS		; POS()
	.word	LAB_SQR		; SQR()
	.word	LAB_RND		; RND()		modified function
	.word	LAB_LOG		; LOG()
	.word	LAB_EXP		; EXP()
	.word	LAB_COS		; COS()
	.word	LAB_SIN		; SIN()
	...
	.word	LAB_ATN		; ATN()
	.word	LAB_PEEK		; PEEK()
	.word	LAB_DEEK		; DEEK()		new function
	.word	LAB_SADD		; SADD()		new function
	.word	LAB_LENS		; LEN()
	.word	LAB_STRS		; STR$()
	.word	LAB_VAL		; VAL()
	.word	LAB_ASC		; ASC()
	.word	LAB_UCASE		; UCASE$()		new function
	.word	LAB_LCASE		; LCASE$()		new function
	.word	LAB_CHRS		; CHR$()
	.word	LAB_HEXS		; HEX$()		new function
	.word	LAB_BINS		; BIN$()		new function
	.word	LAB_BTST		; BITTST()		new function
	.word	LAB_MAX		; MAX()		new function
	.word	LAB_MIN		; MIN()		new function
	.word	LAB_PI		; PI			new function
	.word	LAB_TWOPI		; TWOPI		new function
	.word	LAB_VARPTR		; VARPTR()		new function
	.word	LAB_LEFT		; LEFT$()
	.word	LAB_RIGHT		; RIGHT$()
	.word	LAB_MIDS		; MID$()
...
; Table of Basic keywords. the parsing code has been re written and the
; table can now extend up to 512 bytes.
...
LAB_KEYT
	...
	.cbyte	"NEXT"		; $82 NEXT
	.cbyte	"DATA"		; $83 DATA
	.cbyte	"INPUT"		; $84 INPUT
	.cbyte	"DIM"		; $85 DIM
	.cbyte	"READ"		; $86 READ
	.cbyte	"LET"		; $87 LET
	.cbyte	"DEC"		; $88 DEC		new command
	.cbyte	"GOTO"		; $89 GOTO
	.cbyte	"RUN"		; $8A RUN
	.cbyte	"IF"		; $8B IF
	.cbyte	"RESTORE"	; $8C RESTORE	modified command
	.cbyte	"GOSUB"		; $8D GOSUB
	.cbyte	"RETIRQ"	; $8E RETIRQ	new command
	.cbyte	"RETNMI"	; $8F RETNMI	new command
	.cbyte	"RETURN"	; $90 RETURN
	.cbyte	"REM"		; $91 REM
	.cbyte	"STOP"		; $92 STOP
	.cbyte	"ON"		; $93 ON		modified command
	.cbyte	"NULL"		; $94 NULL		modified command
	.cbyte	"INC"		; $95 INC		new command
	.cbyte	"WAIT"		; $96 WAIT
	.cbyte	"LOAD"		; $97 LOAD		user to do this
	.cbyte	"SAVE"		; $98 SAVE		user to do this
	.cbyte	"DEF"		; $99 DEF
	.cbyte	"POKE"		; $9A POKE
	.cbyte	"DOKE"		; $9B DOKE		new command
	.cbyte	"CALL"		; $9C CALL		new command
	.cbyte	"DO"		; $9D DO		new command
	.cbyte	"LOOP"		; $9E LOOP		new command
	.cbyte	"PRINT"		; $9F PRINT
	.cbyte	"CONT"		; $A0 CONT
	.cbyte	"LIST"		; $A1 LIST
	.cbyte	"CLEAR"		; $A2 CLEAR
	.cbyte	"NEW"		; $A3 NEW
	.cbyte	"WIDTH"		; $A4 WIDTH		new command
	.cbyte	"GET"		; $A5 GET		new command
	.cbyte	"SWAP"		; $A6 SWAP		new command
	.cbyte	"BITSET"	; $A7 BITSET	new command
	.cbyte	"BITCLR"	; $A8 BITCLR	new command
	.cbyte	"IRQ"		; $A9 IRQ		new command
	.cbyte	"NMI"		; $AA NMI		new command
	.cbyte	"TAB("		; $AB TAB(
	...
	.cbyte	"NOT"		; $B0 NOT
	.cbyte	"STEP"		; $B1 STEP
	.cbyte	"UNTIL"		; $B2 UNTIL		new command
	.cbyte	"WHILE"		; $B3 WHILE		new command
	.cbyte	"OFF"		; $B4 OFF		new command
	...
	.cbyte	"^"		; $B9 ^
	.cbyte	"AND"		; $BA AND
	.cbyte	"EOR"		; $BB EOR		new operator
	.cbyte	"OR"		; $BC OR
	.cbyte	">>"		; $BD >>		new operator
	.cbyte	"<<"		; $BE <<		new operator
	.cbyte	">"		; $BF >
	.cbyte	"="		; $C0 =
	.cbyte	"<"		; $C1 <
	...
	.cbyte	"POS"		; $C7 POS
	.cbyte	"SQR"		; $C8 SQR
	.cbyte	"RND"		; $C9 RND		modified function
	.cbyte	"LOG"		; $CA LOG
	.cbyte	"EXP"		; $CB EXP
	.cbyte	"COS"		; $CC COS
	.cbyte	"SIN"		; $CD SIN
	...
	.cbyte	"ATN"		; $CF ATN
	.cbyte	"PEEK"		; $D0 PEEK
	.cbyte	"DEEK"		; $D1 DEEK		new function
	.cbyte	"SADD"		; $D2 SADD		new function
	.cbyte	"LEN"		; $D3 LEN
	.cbyte	"STR$"		; $D4 STR$
	.cbyte	"VAL"		; $D5 VAL
	.cbyte	"ASC"		; $D6 ASC
	.cbyte	"UCASE$"	; $D7 UCASE$	new function
	.cbyte	"LCASE$"	; $D8 LCASE$	new function
	.cbyte	"CHR$"		; $D9 CHR$
	.cbyte	"HEX$"		; $DA HEX$		new function
	.cbyte	"BIN$"		; $DB BIN$		new function
	.cbyte	"BITTST"	; $DC BITTST	new function
	.cbyte	"MAX"		; $DD MAX		new function
	.cbyte	"MIN"		; $DE MIN		new function
	.cbyte	"PI"		; $DF PI		new function
	.cbyte	"TWOPI"		; $E0 TWOPI		new function
	.cbyte	"VARPTR"	; $E1 VARPTR	new function
	.cbyte	"LEFT$"		; $E2 LEFT$
	.cbyte	"RIGHT$"	; $E3 RIGHT$
	.cbyte	"MID$"		; $E4 MID$
...
LAB_BAER
	.word	LAB_NF		;$00 NEXT without FOR
	.word	LAB_SN		;$02 syntax
	.word	LAB_RG		;$04 RETURN without GOSUB
	.word	LAB_OD		;$06 out of data
	.word	LAB_FC		;$08 function call
	.word	LAB_OV		;$0A overflow
	.word	LAB_OM		;$0C out of memory
	.word	LAB_US		;$0E undefined statement
	.word	LAB_BS		;$10 array bounds
	.word	LAB_DD		;$12 double dimension array
	.word	LAB_D0		;$14 divide by 0
	.word	LAB_ID		;$16 illegal direct
	.word	LAB_TM		;$18 type mismatch
	.word	LAB_LS		;$1A long string
	.word	LAB_ST		;$1C string too complex
	.word	LAB_CN		;$1E continue error
	.word	LAB_UF		;$20 undefined function
	.word LAB_LD		;$22 LOOP without DO

; I may implement these two errors to force definition of variables and
; dimensioning of arrays before use.

;	.word LAB_UV		;$24 undefined variable

; the above error has been tested and works (see code & comments below LAB_1D8B)

;	.word LAB_UA		;$26 undimensioned array

LAB_NF	.byte	"NEXT without FOR",$00
LAB_SN	.byte	"Syntax",$00
LAB_RG	.byte	"RETURN without GOSUB",$00
LAB_OD	.byte	"Out of DATA",$00
LAB_FC	.byte	"Function call",$00
LAB_OV	.byte	"Overflow",$00
LAB_OM	.byte	"Out of memory",$00
LAB_US	.byte	"Undefined statement",$00
LAB_BS	.byte	"Array bounds",$00
LAB_DD	.byte	"Double dimension",$00
LAB_D0	.byte	"Divide by zero",$00
LAB_ID	.byte	"Illegal direct",$00
LAB_TM	.byte	"Type mismatch",$00
LAB_LS	.byte	"String too long",$00
LAB_ST	.byte	"String too complex",$00
LAB_CN	.byte	"Can't continue",$00
LAB_UF	.byte	"Undefined function",$00
LAB_LD	.byte	"LOOP without DO",$00

;LAB_UV	.byte	"Undefined variable",$00

; the above error has been tested and works (see code & comments below LAB_1D8B)

;LAB_UA	.byte	"Undimensioned array",$00
