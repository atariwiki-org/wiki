...
void DrawShape(int shape,int Size)
  ...
  line( g.CenterX - Size, g.CenterY - Size,
  ...
  switch (shape)
    ...
    case LEFT_WALL:
      line( g.CenterX - OldSize, g.CenterY - OldSize,
            g.CenterX - Size,    g.CenterY - Size);
      line( g.CenterX - OldSize, g.CenterY + OldSize,
            g.CenterX - Size,    g.CenterY + Size);
      break;
    ...
    case RIGHT_EDGE:
      line( g.CenterX + OldSize, g.CenterY - Size,
            g.CenterX + Size,    g.CenterY - Size);
      line( g.CenterX + OldSize, g.CenterY + Size,
            g.CenterX + Size,    g.CenterY + Size);
      break;
...
void Join( int Cell, int Dir )
  {
  int i, OldPath;

  g.Maze[ Cell ].Walls &= ~Dir;   // clear the wall between them

  // now merge the paths

  OldPath = g.Maze[ Cell + g.Dir[ Dir ] ].Path;

  for( i = g.NumCells; i >= 0; i-- )
    {
    if( g.Maze[ i ].Path == OldPath )
      {
      g.Maze[ i ].Path = g.Maze[ Cell ].Path;
      }
    }
  }
