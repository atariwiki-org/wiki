...
In Basic, we use PEEK(addr) to read a memory location. This example reads the first joystick  
...
The counterparts to @ (FETCH) and C@ (C-FETCH) are ! (STORE) and C! (C-STORE). These words take a value and an address from the stack and store the value at the address. The Stack comments for these words are:  
