...
Unter Forth ist das schon möglich.
...
Nun müssen wir Forth sagen, dass unser neues Wort "HALLO-WELT" nach dem Start unseres zu speichernden Programms direkt ausgeführt werden soll. Dies geschieht, indem wir die Ausführungsaddresse des Wortes HALLO-WELT dem "verzögerten" (deferred) Wort 'COLD zuweisen ('COLD = ausgesprochen "tick cold"). Die Ausführungaddresse unseres neuen Wortes holen wir uns mit dem Wort ' (tick), die Zuweisung geschieht mit dem Wort "IS":
...
Das Wort "SAVE" sichert (im Hauptspeicher) unsere Erweiterungen zum Forth (die danach nicht mehr im Speicher gelöscht werden können). SAVE-SYSTEM <dateiname> speichert dann das Programm als COM-Datei ab.
...
Warum ist das "HELLO.COM" Programm so groß? Weil SAVE-SYSTEM immer das gesamte Forth-System speichert, inklusive aller Worte, die zuvor bereits im Forth definiert waren. Später werde ich Euch zeigen, wie wir das Programm schlanker bekommen.
