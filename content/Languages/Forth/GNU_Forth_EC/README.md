...
On desktop-computer CPUs there is not much difference between hand-coded primitives and the generated code of a C-Compiler, so this was an easy way of making the system capable of running on a wide range of target platforms.  
Due to the flexible cross-compiler, it was easy to use real assembler primitives instead of C. But there were problems, too: During the years, GForth has migrated to a big 'PC-forth'-system, so some work was necessary to strip down the system.  
